...
//                        std::cout << "=================================================================" << std::endl;
//                        std::cout << "scounts[" << i << "] is " << scounts[i] << std::endl;
//                        std::cout << "displs[" << i << "] is " << displs[i] << std::endl;
//                        std::cout << "=================================================================" << std::endl;
        }
    }
...
void transpose_bcast_vector(const int n, double* col_vector, double* row_vector, MPI_Comm comm)
{
    ...
    // vector allocations are not here. didn't test this yet.
...
//    *  broadcasts the message among it's column using a column sub-communicator.
    ...
    if(row_rank == 0 && col_rank != 0) // sender processor, exclude (0,0) it doesn't have to send
    {
        MPI_Send(col_vector, scount, MPI_DOUBLE, col_rank, 0, row_comm);
    }
    else if(row_rank == col_rank && col_rank != 0)//receiver processor, again exclude (0,0)
    {
